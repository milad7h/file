...
if(isset($_REQUEST['type'])){
    ...
    $sql = "SELECT * FROM buy WHERE type LIKE ? and user_id = $userid and area >= $area and total_price >= $total_price ORDER BY  id DESC";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) > 0){
                ...
                while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                    echo "<div class=\"my-sells  my-sell-".$row['id'] . " \">
                    <div class=\"row mb-2 buyf" .$row['id'].  " \" style=\"direction: ltr;\">
                                                <button type=\"button\" class=\"btn  btn-danger \" id=".$row['id']. " onclick=\"del_my_sell(this.id)\"> <span uk-icon=\"icon: trash; ratio: 0.9\"></span></button>
                                                <button type=\"button\" class=\"btn btn-info buy_edit_enable\" style=\"margin-left: 10px\" id=".$row['id']. " onclick=\"edit_my_sell(this.id)\"> <span uk-icon=\"icon: pencil; ratio: 0.9\"></span></button>
                                                        
                                            </div>

                    <div class=\"case my-4 \" id=\"buyf".$row['id'] . "\">

                       <div class=\"row buyf" .$row['id'].  " rent-fade\">
